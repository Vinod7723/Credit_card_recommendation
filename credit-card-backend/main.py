...
def create_order_route():
    ...
    try:
        ...
        if order_number:
            return jsonify({"message": f"Order created successfully with order number {order_number}."})
        else:
...
def cancel_order_route():
    ...
    try:
        if cancel_order_with_openai(order_number):
            return jsonify({"message": f"Order {order_number} has been successfully canceled."})
        else:
